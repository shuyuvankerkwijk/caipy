...
from tracking.utils.exceptions import TrackingError, SafetyError, MQTTError, ValidationError, OperationError, ConfigurationError, TimeoutError
    ...
    def __call__(self, progress_info: ProgressInfo) -> None:
        ...
        if progress_info.antenna == self.antenna:
            if progress_info.is_complete:
                print(f"\n{self.antenna} {progress_info.operation_type.value}: Complete")
            elif progress_info.error:
                print(f"\n{self.antenna} {progress_info.operation_type.value}: Error - {progress_info.error}")
            else:
                # Only print if percent changed significantly
                if progress_info.percent_complete - self.last_percent >= 5.0:
                    print(f"{self.antenna} {progress_info.operation_type.value}: {progress_info.percent_complete:.1f}% - {progress_info.message}")
...
def track_and_park(ant: str, ra_hrs: float = None, dec_deg: float = None, pm_ra: float = 0, pm_dec: float = 0, 
    ...
    """
    if ant not in ["N", "S"]:
        logger.error("Error: Antenna must be N (North) or S (South)")
        return False
    ...
    try:
        ...
        if progress_callback is None:
            progress_callback = CLIProgressCallback(ant)
        
        # Run tracking with optional slewing and parking
        success = tracker.run_track(ant, source, duration_hours, slew=slew, park=park, progress_callback=progress_callback)
...
def slew_to_position(ant: str, ra_hrs: float = None, dec_deg: float = None, pm_ra: float = 0, pm_dec: float = 0, 
    ...
    try:
        ...
        if az is not None and el is not None:
            ...
            success = tracker.run_slew(ant, az=az, el=el, progress_callback=progress_callback)
            ...
            success = tracker.run_slew(ant, source=source, progress_callback=progress_callback)
        else:
...
def park_telescope(ant: str, progress_callback: ProgressCallback = None):
    ...
    try:
        ...
        # Park the telescope
        success = tracker.run_park(ant, progress_callback=progress_callback)
